################################################################################
# This function creates neural network structure plots.
#
# Original code is available here:
#   https://gist.github.com/fawda123/5086859
#
# The amount of spacing between units has been increased slightly, and some 
# minor bugfixes for our data set have been implemented.
#
################################################################################
...
plot.nnet<-function(mod.in,nid=T,all.out=T,all.in=T,wts.only=F,rel.rsc=5,circle.cex=5,node.labs=T,
  ...
  get.ys<-function(lyr){
    if (lyr == max(struct)) {
      ...
      spacing<-diff(c(0*diff(y.range),0.9*diff(y.range)))/lyr * 0.9
    }
...
#    else all.out<-which(y.names==all.out)
    else all.out<-match(all.out, y.names)
